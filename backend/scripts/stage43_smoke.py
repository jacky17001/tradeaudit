#!/usr/bin/env python3
"""Stage 43 smoke: Audit Report endpoint checks."""
...
def main() -> int:
    print("\n" + "=" * 72)
    print("Stage 43: Audit Report - Smoke Test")
    print("=" * 72)
    ...
    try:
        resp = requests.get(f"{BASE_URL}/api/audit-report?kind=strategy", headers=headers, timeout=8)
        payload = resp.json() if resp.status_code == 200 else {}
        ok = resp.status_code == 200 and payload.get("kind") == "strategy" and has_required(payload)
        results.append(test_print("Strategy report shape", ok))
    except Exception as exc:
        results.append(test_print("Strategy report shape", False, str(exc)))

    try:
        resp = requests.get(f"{BASE_URL}/api/audit-report?kind=account", headers=headers, timeout=8)
        payload = resp.json() if resp.status_code == 200 else {}
        ok = resp.status_code == 200 and payload.get("kind") == "account" and has_required(payload)
        results.append(test_print("Account report shape", ok))
    except Exception as exc:
        results.append(test_print("Account report shape", False, str(exc)))

    try:
        resp = requests.get(f"{BASE_URL}/api/audit-report", headers=headers, timeout=8)
        payload = resp.json() if resp.status_code == 200 else {}
        ok = (
        ...
        results.append(test_print("Combined report shape", ok))
    except Exception as exc:
        results.append(test_print("Combined report shape", False, str(exc)))

    try:
        resp = requests.get(f"{BASE_URL}/api/audit-report?kind=bad", headers=headers, timeout=8)
    ...
    try:
        resp = requests.get(f"{BASE_URL}/api/audit-report", timeout=8)
